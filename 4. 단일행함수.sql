...
CREATE TABLE CHAR_COMPARE (
    sn CHAR(10), --char : 바이트를 지정하면 모자란 부분을 공백으로 채움
    char_4 CHAR(4),
    char_6 CHAR(6)
);
...
-- varchar : 공백을 채우지 않고 할당된 바이트대로 비교함
...
;
 ...
 SELECT
    * FROM varchar_compare
    where sn = '101'
        AND varchar_6 = 'SQLD' -- false (유효성검사 할 때 사용)
;

-- 단일행 함수 : 행 마다 호출,적용(인출된 모든 행 확인가능)
SELECT
    emp_nm,
    SUBSTR(addr, 1, 3)
    FROM tb_emp
...
FROM dual; --쓰레기테이블, 실험용데이터

SELECT
    ASCII('ㅎ'), -- 문자를 아스키코드로
    CHR(10)     -- 아스키코드를 문자로 (공백값)
FROM dual;

SELECT 
    --'SQL' || 'DEVELOPER' -- oracle
    CONCAT('SQL', 'Developer'),   -- 문자열을 결합 (except oracle)
    SUBSTR('SQL Developer', 1, 3), -- 문자열 자름 1번부터 <3글자>자름 (첫글자가 1번)
    LENGTH('HELLO WORLD'),         -- 문자열의 길이
    TRIM('    HI   '),              -- 좌우 공백 제거
    LTRIM('  HELLO   '),  
    RTRIM('  HELLO   '),
    LTRIM('HELLOhello', 'HELL') -- 왼쪽에서 발견된 HELL를 제거
FROM dual;
...
SELECT
    ...
    TO_CHAR(SYSDATE - (1/24) * 6, 'YY/MM/DD PM HH12:MI:SS') AS "날짜와 시간2"
FROM dual;
...
SELECT 
    ...
    CASE WHEN sal_cd = '100001' THEN '기본급여'
         ...
         ELSE '기타'
     END sal_name --AS 와 같은 개념
FROM tb_sal;


-- Simple expression : 한번에표현하기
SELECT 
    sal_cd, 
    sal_nm,
    CASE sal_cd 
...
--NULL이 아닐때까지 다음값, 다다음값까지 계속 확인해주는 COALESCE
...
SELECT 
     ...
     ADDR 
FROM tb_emp
WHERE emp_nm = '이정직2'; --NULL 값이 있어야 NULL 조회 가능 : 공집합 개념
...
SELECT 
    -- emp_nm
    -- MAX(emp_nm)-- 최대값 찾기 (값이 있는 경우를 조회 가능ㅋㅋ)
    -- NVL(emp_nm, '존재안함') AS emp_nm
    NVL(MAX(emp_nm), '존재안함') AS emp_nm
FROM tb_emp
WHERE emp_nm = 'ㅅ,ㅇㅇ'; --못찾았을때 공집합이 아닌  null을 얻어냈을 때 존재안함으로 변환
