...
ALTER TABLE tb_emp DROP CONSTRAINT fk_tb_emp01; -- 외래키 해제
...
SELECT
    ...
    D.dept_nm
FROM tb_emp E
INNER JOIN tb_dept D --부서가 있는 emp_nm만 조회가 된다. 
...
-- 사원명과 부서명 조회 : 사원정보는 일단 모두 조회
SELECT
    E.emp_nm,
    D.dept_nm
     FROM tb_emp E LEFT OUTER JOIN tb_dept D 
     -- ** join 을 기준으로 ** 왼쪽 정보를 다 보여주고
     -- 오른쪽 정보는 아래 조건에 맞게 보여줘
     ON E.dept_cd = D.dept_cd
    ;
    ...
    SELECT
        *
     FROM tb_emp E LEFT OUTER JOIN tb_dept D --(ALSO SAME AS LEFT JOIN)
     -- ** join 을 기준으로 ** 왼쪽 정보를 다 보여주고
     -- 오른쪽 정보는 아래 조건에 맞게 보여줘
     ON E.dept_cd = D.dept_cd
    ;
...
SELECT
    ...
    D.dept_nm
     ...
     -- ** join 을 기준으로 ** 오른쪽 정보를 다 보여주고
     -- 왼쪽 정보는 아래 조건에 맞게 보여줘
     ON E.dept_cd = D.dept_cd
    ;
...
-- #FULL OUTER JOIN
-- 왼쪽 조인테이블, 오른쪽 조인테이블을 모두 보여주고 
-- 매칭되면 매칭데이터까지 합쳐서 보여줌
SELECT 
...
-- 사원이 존재하는 부서 14개
-- inner JOIN : 41건
-- left JOIN : INNER 41 + 전체사원 (부서 없는 사원 포함) : 46
-- RIGHT JOIN : INNER 41 + 전체부서 (사원 없는 부서 포함): 43
-- FULL : INNER + 5(새로운 사원) + 2(새로운 부서) = 48건 

-- # SELF JOIN
-- 1. 하나의 테이블에서 자기 자신의 테이블끼리 조인하는 기법입니다.
-- 2. 자기 자신 테이블에서 pk와 fk로 동등조인합니다.
-- 2-1. 자기자신 테이블을 복제한 뒤 cross join 한다.
-- 2-2. 그 테이블에서 조건을 걸어서  SELF JOIN 
...
SELECT
    ...
    B.emp_nm "직속상사"
     FROM tb_emp A
     LEFT JOIN tb_emp B
     ON a.direct_manager_emp_no = b.emp.no
     ORDER BY a.emp_no
;
