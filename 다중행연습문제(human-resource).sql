...
--## SELECT FROM WHERE GROUPBY 순서 꼭 기억, 공식문서 참조
-- https://docs.oracle.com/cd/B13789_01/server.101/b10759/statements_10002.htm
-- 1. employees테이블에서 각 사원의 부서별 부서 번호(department_id)와 
--    평균 급여(salary)를 조회하세요. 
--    단, 부서번호가 null이면 0으로 변경하여 조회세요.
SELECT 
  NVL(department_id, 0) AS dep_idㅌ 
...
FROM employees
GROUP BY department_id
HAVING AVG(salary) > 8000 --그룹화 이후 HAVING
