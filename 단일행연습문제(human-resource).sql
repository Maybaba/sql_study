-- 1. employees 테이블에서 모든 사원의 first_name을 대문자로 변환하여 출력하세
SELECT
    UPPER(first_name)
     FROM employees
...
-- 6. employees 테이블에서 manager_id가 NULL인 사원의 이름first_name과 직책job_id을 출력하세요.
SELECT
    first_name AS 이름,
    job_id AS 직책
     FROM employees
...
SELECT
    LENGTH(job_title) AS job_title_length
    FROM jobs
    ;
...
SELECT
    SUBSTR(last_name, 1, 1) || first_name, --성의 첫 글자
    CONCAT(SUBSTR(last_name, 1, 1), first_name) 
     FROM employees
    ;

-- 10. locations 테이블에서 city의 이름을 역으로 출력하세요. 스펠링 뒤집기
SELECT
    REVERSE(city)
     FROM locations
    ;

-- 11. employees 테이블에서 각 사원의 salary를 100달러 단위로 올림하여 출력하세요. 100달러 단위 올림
SELECT
...
SELECT --실제 인상시 update
    salary + (salary * 0.1) AS "급여 10%인상"
    FROM employees
    ;
